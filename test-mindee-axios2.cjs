...
async function test() {
  ...
  try {
    const enqueueRes = await axios.post(
      `https://api.mindee.net/v1/products/mindee/business_card/v1/predict`, 
      formData,
      {
        headers: { Authorization: `Token ${apiKey}`, ...formData.getHeaders() }
      }
    );
    console.log('Success API v1:', enqueueRes.data);
  } catch (err) {
    console.error('Error API v1:', err.message, err.response?.data);
  }
}
test();
